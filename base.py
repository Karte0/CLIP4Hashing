...
from utils import get_plotter, AverageMeter
...
class TrainBase(object):
    def __init__(self, name, data_name, bit, batch_size, visdom=True, cuda=True):
        ...
        self.checkpoint_dir = os.path.join(
            '../../../../files/大四下/毕设/DCMH-0407/deep-cross-modal-hashing-master/torchcmh', 'checkpoints', self.name, data_name)
    ...
    def valid(self, epoch):
        ...
        if mapt2i + mapi2t >= self.max_mapi2t + self.max_mapt2i:
            ...
            self.img_model.save_dict(os.path.join(self.checkpoint_dir, str(self.bit) + '-' + self.img_model.module_name + str(datetime.datetime.now()) + '.pth'))
            self.txt_model.save_dict(os.path.join(self.checkpoint_dir, str(self.bit) + '-' + self.txt_model.module_name + str(datetime.datetime.now()) + '.pth'))
    ...
    def get_codes(img_model, txt_model, dataset, bit, batch_size, cuda=True):
        ...
        for data in tqdm(dataloader):
            ...
            img_hash = img_model(img)
            txt_hash = txt_model(txt)
            # img_hash = torch.img_model(img)
            # txt_hash = torch.txt_model(txt)
    ...
    def save_code(self, epoch):
        ...
        path = os.path.join(self.checkpoint_dir, str(self.bit) + str(datetime.datetime.now()) + '-hash.mat')
